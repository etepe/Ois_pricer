...
// ─── BOND DATA FROM EXCEL (last prices, sorted by maturity) ─────────
const BONDS=[
...
function generateCFs(bond,vd,oisNodes){
  const matDate=pd(bond.mat);
  const totalDays=db(vd,matDate);
  if(totalDays<=0)return[];

  if(bond.freq===0||bond.cpn===0){
    // Zero coupon: just 100 at maturity
    return[{days:totalDays,cf:100}];
  }

  const periodMonths=bond.freq===4?3:6;
  const periodDaysApprox=bond.freq===4?91:182;
  ...
  const cfs=[];
  if(bond.type==="flt"){
    // Floating: CFs from OIS forward rates (DF ratios)
    for(let i=0;i<dates.length;i++){
      const dCurr=dates[i];
      const dPrev=i>0?dates[i-1]:0;
      const dfCurr=idf(oisNodes,dCurr);
      const dfPrev=idf(oisNodes,dPrev);
      const fwd=(dfPrev/dfCurr-1)*100;
      cfs.push({days:dCurr,cf:i===dates.length-1?fwd+100:fwd});
    }
  } else {
    // Fixed coupon: cpn per period = cpn_annual / freq * period_days/365... 
    // Actually Excel uses fixed amount = cpn/freq per period
    const cpnPerPeriod=bond.cpn/bond.freq;
    for(let i=0;i<dates.length;i++){
      cfs.push({days:dates[i],cf:i===dates.length-1?cpnPerPeriod+100:cpnPerPeriod});
    }
  }
  return cfs;
}
...
function solveZSpread(cfs,oisNodes,targetPrice,periodApprox){
  // DF with spread: use period-based compounding matching Excel
  // DF(d,s) = 1/(1+(r+s)/365*P)^(d/P) where P=91 or 182
  // r = ((1/DF_ois)^(P/d) - 1)*365/P
  function pvWithSpread(spread){
    let pv=0;
    for(const{days,cf}of cfs){
      if(days<=0)continue;
      const dfOis=idf(oisNodes,days);
      if(dfOis<=0)continue;
      // Convert OIS DF to period-compound rate
      const r=(Math.pow(1/dfOis,periodApprox/days)-1)*365/periodApprox;
      const dfSpread=1/Math.pow(1+(r+spread)/365*periodApprox,days/periodApprox);
      pv+=cf*dfSpread;
    }
    return pv;
  }
  ...
  // Ensure brackets
  if(pvWithSpread(lo)<tgt){[lo,hi]=[-1,1];}
  for(let i=0;i<100;i++){
    mid=(lo+hi)/2;
    const pv=pvWithSpread(mid);
    if(Math.abs(pv-tgt)<0.0001)break;
    if(pv>tgt)lo=mid;else hi=mid;
  }
  return mid;
}
...
function priceBonds(bonds,oisNodes,vd){
  return bonds.map(bond=>{
    const matDate=pd(bond.mat);
    const totalDays=db(vd,matDate);
    if(totalDays<=0)return{...bond,totalDays:0,zspread:null,pvFlat:null};

    const periodApprox=bond.freq===4?91:182;
    const cfs=generateCFs(bond,vd,oisNodes);
    if(!cfs.length)return{...bond,totalDays,zspread:null,pvFlat:null};

    // PV flat (no spread)
    let pvFlat=0;
    for(const{days,cf}of cfs){
      if(days<=0)continue;
      const df=idf(oisNodes,days);
      pvFlat+=cf*df;
    }

    // Solve Z-spread to match last price
    const zspread=solveZSpread(cfs,oisNodes,bond.last,periodApprox);

    return{...bond,totalDays,zspread:zspread*100,pvFlat,cfs};
...
function CV({v,f=1,s="%",n}){const c=n?B.tx:v>0.005?B.rd:v<-0.005?B.gn:B.tm;return<span style={{color:c,fontFamily:"'JetBrains Mono',monospace",fontSize:"12px"}}>{v>0.005?"+":""}{v.toFixed(f)}{s}</span>;}
    ...
    <div style={{minHeight:"100vh",background:B.bg,color:B.tx,fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif"}}>
      <div style={{background:B.sf,borderBottom:`1px solid ${B.bd}`,padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"12px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
          ...
          <div style={{fontSize:"10px",color:B.tm,fontFamily:"'JetBrains Mono',monospace"}}>T+1 · Act/365 · Quarterly Bootstrap · Bond Z-Spread</div></div>
        </div>
      ...
      <div style={{padding:"16px 20px"}}>
        {tab==="bonds"&&<BondTab bonds={bondResults}/>}
        {tab==="ppk"&&<PPKTab ppk={pw} tlref={tlref} cum={cum}/>}
        {tab==="scenario"&&<ScTab ppk={pw} spot={bondResults} sD={sD} rD={rD} hd={hd} quotes={quotes} qt={qt} vd={bs.vd} nodes={bs.nodes}/>}
        {tab==="market"&&<MktTab q={quotes} uQ={uQ} qt={qt} n={bs.nodes}/>}
      </div>
      <div style={{padding:"8px 20px",borderTop:`1px solid ${B.bd}`,fontSize:"9px",color:B.tm,fontFamily:"'JetBrains Mono',monospace",textAlign:"right",letterSpacing:"1px"}}>FETM RESEARCH — TLREF OIS + BOND ENGINE v2.0</div>
    </div>
  );
}

// ─── BOND TAB ────────────────────────────────────────────────────────
function BondTab({bonds}){
  const maxSpread=Math.max(...bonds.filter(b=>b.zspread!==null).map(b=>Math.abs(b.zspread)),1);
  return<div>
    <div style={{marginBottom:"12px",fontSize:"12px",color:B.tm}}>
      Z-Spread over OIS curve (mid). Bonds sorted by days to maturity.
    </div>
    <div style={{overflowX:"auto"}}>
      <table style={{borderCollapse:"collapse",width:"100%"}}><thead><tr>
        {["ISIN","Maturity","Days","Cpn","Freq","Type","Last Px","OIS PV","Z-Spread",""].map(h=>
          <th key={h} style={{...hs,textAlign:["ISIN","Maturity","Type"].includes(h)?"left":"right"}}>{h}</th>)}
      </tr></thead><tbody>
        {bonds.map((b,i)=>{
          const spreadColor=b.zspread===null?B.tm:b.zspread>0.5?B.rd:b.zspread<-0.5?B.gn:B.tx;
          const barW=b.zspread!==null?Math.abs(b.zspread)/maxSpread*100:0;
          const typeLabel=b.type==="zcb"?"ZCB":b.type==="flt"?"FLT":"FIX";
          const typeColor=b.type==="zcb"?B.am:b.type==="flt"?B.cy:B.bl;
          return<tr key={b.isin+i} style={{background:i%2?`${B.sa}44`:"transparent"}}>
            <td style={{...cs,fontWeight:600,color:B.tx,fontSize:"11px"}}>{b.isin}</td>
            <td style={{...cs,color:B.tm}}>{b.mat}</td>
            <td style={{...cs,textAlign:"right",color:B.tm}}>{b.totalDays}</td>
            <td style={{...cs,textAlign:"right"}}>{b.cpn?b.cpn.toFixed(1)+"%":"—"}</td>
            <td style={{...cs,textAlign:"right",color:B.tm}}>{b.freq||"—"}</td>
            <td style={{...cs}}><span style={{color:typeColor,fontSize:"10px",fontWeight:700,padding:"1px 5px",background:`${typeColor}15`,borderRadius:"3px"}}>{typeLabel}</span></td>
            ...
            <td style={{...cs,textAlign:"right"}}>
              <span style={{color:spreadColor,fontWeight:600}}>
                {b.zspread!==null?(b.zspread>0?"+":"")+b.zspread.toFixed(1)+"%":"—"}
              </span>
            </td>
            <td style={{...cs,width:"80px",padding:"6px 4px"}}>
              {b.zspread!==null&&<div style={{width:"80px",height:"12px",background:B.sa,borderRadius:"2px",overflow:"hidden",display:"flex",justifyContent:b.zspread>0?"flex-start":"flex-end"}}>
                <div style={{width:`${Math.max(barW,3)}%`,height:"100%",background:b.zspread>0?B.rd:B.gn,borderRadius:"2px",opacity:.6}}/>
              </div>}
            </td>
          </tr>;
        })}
    ...
    <div style={{marginTop:"16px",display:"flex",gap:"24px",flexWrap:"wrap"}}>
      <StatBox label="Total Bonds" value={bonds.length} />
      <StatBox label="Avg Z-Spread" value={(bonds.filter(b=>b.zspread!==null).reduce((s,b)=>s+b.zspread,0)/bonds.filter(b=>b.zspread!==null).length).toFixed(2)+"%"} />
      <StatBox label="Widest" value={bonds.reduce((w,b)=>b.zspread!==null&&Math.abs(b.zspread)>Math.abs(w)?b.zspread:w,0).toFixed(1)+"%"} />
    </div>
  </div>;
}

function StatBox({label,value}){
  return<div style={{background:B.sf,border:`1px solid ${B.bd}`,borderRadius:"6px",padding:"10px 16px",borderLeft:`3px solid ${B.bl}`,minWidth:"100px"}}>
    <div style={{fontSize:"9px",color:B.tm,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:"4px"}}>{label}</div>
    <div style={{fontSize:"16px",fontWeight:700,color:B.tx,fontFamily:"'JetBrains Mono',monospace"}}>{value}</div>
  </div>;
}
...
function PPKTab({ppk,tlref,cum}){
  return<div>
    ...
    <div style={{overflowX:"auto"}}><table style={{borderCollapse:"collapse",width:"100%"}}><thead><tr>
      ...
      {ppk.map((p,i)=>{const chg=i===0?p.ir-tlref:p.ir-ppk[i-1].ir;
...
// ─── SCENARIO TAB ────────────────────────────────────────────────────
function ScTab({ppk,sD,rD,hd,quotes,qt,vd,nodes}){
  return<div style={{display:"flex",gap:"20px",flexWrap:"wrap"}}>
    ...
    <div style={{flex:"1 1 350px",minWidth:"280px"}}>
      ...
      <table style={{borderCollapse:"collapse",width:"100%"}}><thead><tr>
        {["Tenor","Rate"].map(h=><th key={h} style={{...hs,textAlign:h==="Tenor"?"left":"right"}}>{h}</th>)}
      </tr></thead><tbody>
        {quotes.map((q,i)=>{const r=qt==="bid"?q.bid:qt==="ask"?q.ask:(q.bid+q.ask)/2;
          return<tr key={q.t} style={{background:i%2?`${B.sa}44`:"transparent"}}>
            <td style={{...cs,fontWeight:600}}>{q.t}</td>
            <td style={{...cs,textAlign:"right",color:B.bl}}>{r.toFixed(2)}%</td>
          </tr>;
        })}
      </tbody></table>
    </div>
...
function MktTab({q,uQ,qt,n}){
  return<div style={{display:"flex",gap:"20px",flexWrap:"wrap"}}>
    <div style={{flex:"1 1 480px"}}>
      ...
      <table style={{borderCollapse:"collapse",width:"100%"}}><thead><tr>
        {["Tenor","Bid","Ask","Mid","Used"].map(h=><th key={h} style={{...hs,textAlign:h==="Tenor"?"left":"right"}}>{h}</th>)}
      </tr></thead><tbody>
        {q.map((x,i)=>{const mid=(x.bid+x.ask)/2,used=qt==="bid"?x.bid:qt==="ask"?x.ask:mid;
    ...
    <div style={{flex:"1 1 320px"}}>
      ...
      <table style={{borderCollapse:"collapse",width:"100%"}}><thead><tr>
        {["Days","Tenor","DF","Zero Rate"].map(h=><th key={h} style={{...hs,textAlign:h==="Days"||h==="Tenor"?"left":"right"}}>{h}</th>)}
      </tr></thead><tbody>
        {n.filter(x=>x.d>0).map((x,i)=>{const zr=(1/x.f-1)*365/x.d*100;
